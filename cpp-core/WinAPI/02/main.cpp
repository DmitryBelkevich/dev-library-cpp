...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCommandLine, int nCommandShow) {
    ...
    ShowWindow(hMainWindow,
               nCommandShow);            // отображение окна. Параметры функции: дескриптор окна, стиль отображения окна (При первом запуске окна должен быть равен последнему параметру функции WinMain(), а в последующие разы можно вписывать свои данные)
    UpdateWindow(hMainWindow);                    // обновление окна. Параметр функции: дескриптор окна

    // цикл обработки сообщений (извлекает сообщения из очереди, посылаемые фу-циями, ОС)
